...
Integration tests for StarlightGraph backed by a real on-disk SQLite
...
(starlight.query.query_cache.store_accepts_prepared_query returns True for
it), so queries are evaluated locally, in-process, where starlight's custom
...
@pytest.fixture
def sg(db_path):
    """Fresh StarlightGraph backed by a new SQLite database (rdf-1.1 mode)."""
    g = StarlightGraph(store='SQLAlchemy', identifier=GRAPH_URI)
...
def _reopen(db_path) -> StarlightGraph:
    """A fresh StarlightGraph instance opened against an existing database -
...
# decomposition mechanism starlight.query.remote_decompose implements for a
