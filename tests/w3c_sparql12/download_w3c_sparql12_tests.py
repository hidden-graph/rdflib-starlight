...
separate test suite from starlight's own tests/w3c/ (Turtle-1.2-only, a
...
Populates tests/w3c_sparql12/data/ for this repo's own
test_w3c_sparql12_eval.py, which runs the *original* W3C query text directly
through StarlightGraph (no sparql1_2_to_rdf translation pipeline involved at
all) and compares against the suite's own official expected results - the
"does starlight itself correctly execute these queries" check, independent
of anything the downstream sparql1_2_to_rdf project's own translation adds
or might get wrong. (The same fixture data, and the same fetch logic, is
also vendored into the sparql1_2_to_rdf project's own tests/w3c_sparql12/ -
copied rather than shared via a cross-repo path, since starlight's own test
...
these four are the ones currently fetched; the rest are not fetched yet):
