...
Adapted from (not imported from) the downstream ``sparql1_2_to_rdf``
project's own ``tests/w3c_sparql12/harness.py`` - duplicated rather than
imported across repos, since starlight's own test suite shouldn't depend on
...
themselves ordinary term JSON) to build ``starlight.model.triple.TripleTerm``
values - the same type ``StarlightGraph.query()`` itself returns for a
...
def data_format(entry: TestEntry) -> str:
    """The correct StarlightGraph.parse format= for entry.data_file, keyed
    off its actual file extension - StarlightGraph.parse's format= isn't
...
# ---------------------------------------------------------------------------
#
# Loading itself goes through the ordinary public StarlightGraph/
# StarlightDataset.parse() path (see test_w3c_sparql12_eval.py's
...
# bugs (StarlightGraph._native_add() always wrapping writes in
...
# fixed at the source (starlight_graph.py's _native_scoped(),
...
def _parse_json_term(term: dict):
    ...
    if kind == "literal":
        ...
        # starlight.backends.native._parse_json_term, which already handles
...
def _canon_term(v) -> str:
    ...
    sparql1_2_to_rdf project's own copy of this harness: `TripleTerm.n3()`
    (starlight/model/triple.py) falls back to a `_namespace_manager`
    ...
    coming back from `StarlightGraph.query()` renders prefixed
    ...
    starlight.model.dirlangstring's module docstring - deliberately, so it
...
def _opaque_key(v) -> str | None:
    ...
    anonymous reifier), and a URIRef under starlight's own RR_NS (what
    StarlightGraph/StarlightDataset actually produce for one - confirmed in
    starlight/model/encoding.py: anonymous ``~``/``{| |}`` reifiers are
...
def bindings_match(actual: list[dict], expected: list[dict]) -> bool:
    ...
    where StarlightDataset labels a blank node ``_:bc_0_b0_b`` while the
    ...
    through StarlightGraph/StarlightDataset (see _opaque_key) - both cases
