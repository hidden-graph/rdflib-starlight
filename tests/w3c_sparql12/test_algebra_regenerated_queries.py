...
cleanly against StarlightGraph's in-memory backend (see
...
*algebra-regenerated* text the downstream sparql1_2_to_rdf project's own
...
(starlight/query/sparql12_to_11.py) trips on the regenerated phrasing
...
(sparql1_2_to_rdf.parse12.prepare_query_12 -> query_to_rdf -> rdf_to_query
-> sparql1_2_to_rdf.serialize12.translate_algebra_12), and hardcoded here
...
def _new_graph(data_format: str | None) -> StarlightGraph | StarlightDataset:
...
def test_regenerated_select_matches_original(name, data, fmt, original, regenerated):
    ...
    assert bindings_match(original_bindings, regenerated_bindings), (
        f"[{name}] original and regenerated disagree against starlight's in-memory backend.\n"
        f"original Q:\n{original}\nregenerated Q':\n{regenerated}\n"
