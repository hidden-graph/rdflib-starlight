"""Parity check: StarlightDataset(sparql_pipeline='algebra_ir') against the
exact same SELECT/ASK/CONSTRUCT/UPDATE scenarios tests/unit/test_dataset_query.py
already checks for the legacy pipeline - same TriG fixture, same queries,
same expected graph state. Confirms Phase 2's StarlightDataset extension
(query() and update(), both string-object and pre-built-Query-object paths)
holds for the harder multi-graph/Dataset case, not just single-graph
StarlightGraph (see tests/unit/test_algebra_ir_pipeline.py for that).

Deliberately a separate file/copy rather than a parametrize-over-both-
pipelines harness in test_dataset_query.py itself, for the same reason as
test_w3c_sparql12_algebra_ir.py: running algebra_ir and legacy-pipeline
tests in one pytest process is a confirmed way to corrupt sparql1_2_to_rdf's
grammar installation (see docs/testing-strategy.md tier 5). Two independent
suites asserting the same expected state is an equally strong parity claim
without that risk.
...
@pytest.fixture
def ds():
    d = StarlightDataset(sparql_pipeline='algebra_ir')
    d.parse(data=TRIG, format='trig12')
    return d


# ---------------------------------------------------------------------------
# SELECT - ground triple-term pattern, across graphs, restoration, functions
# ---------------------------------------------------------------------------
...
class TestSelectFunctions:
    ...
    def test_istriple_filter(self, ds):
        # isTRIPLE is the spec name (17.4.6); isTripleTerm is a
        # starlight-legacy-rewriter-only alias sparql1_2_to_rdf's grammar,
        # built directly against the spec, never needed to support - see
        # sparql12_to_11.py's own comment at its isTRIPLE handling.
        q = f"""
        PREFIX ex: <{EX}>
        PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
        SELECT ?tt WHERE {{
...
class TestUpdateWithTripleTerm:
    ...
    def test_cross_graph_update(self, ds):
        """WHERE reads graph2, INSERT writes graph1 - confirms the lowered
        Update object's pattern/template graphs aren't conflated."""
        q = f"""
        PREFIX ex: <{EX}>
        PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
        INSERT {{
...
def test_invalid_pipeline_value_rejected():
    with pytest.raises(ValueError):
        StarlightDataset(sparql_pipeline='bogus')
