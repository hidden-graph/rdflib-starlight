"""
starlight.parsers.errors

TurtleSyntaxError — raised by the hand-rolled Turtle 1.2 parser
(starlight/parsers/{turtle_parser,lexer,syntax}.py) for malformed input.
...
surrounding source text), so starlight's own hand-written parser fails as
...
same way (see split_statements_with_lines in starlight.parsers.syntax for
...
class TurtleSyntaxError(SyntaxError):
    ...
    line -- 1-based line number in the original source document, or None if
            not yet known (see StarlightTurtleParser.parse(), which fills
