...
``sparql1_2_to_rdf``'s own grammar, which extends it) has no notion of this
...
See ``starlight.model.conformance`` for what happens with the extracted
label (a warning, never a hard error, if it doesn't match what the query
actually uses), and ``starlight.backends.native.check_native_version_conformance``
...
def contains_triple_term(node) -> bool:
    ...
    conformance checking (see ``starlight.model.conformance``). More
    ...
    """
    from sparql1_2_to_rdf.triple_term import TripleTermNode
