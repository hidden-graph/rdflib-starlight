...
confirmed independently of any starlight/pyshacl-starlight code with plain
...
def patch_multiplicative_expression_type_promotion() -> bool:
    ...
    ``rdflib.Graph().query()`` (7.6.0), no starlight/pyshacl-starlight
    ...
    ``pyshacl_starlight/validator.py`` patch idiom - returns ``False``
    ...
    try:
        ...
        if getattr(original_evalfn, "_starlight_multiplicative_type_promotion_patch", False):
        ...
        _patched_multiplicative_expression._starlight_multiplicative_type_promotion_patch = True  # type: ignore[attr-defined]
...
def patch_decimal_result_lexical_form() -> bool:
    ...
    Idempotent and defensive, matching the established
    ``pyshacl_starlight/validator.py`` patch idiom.
    """
    ...
    try:
        ...
        if getattr(comps["Builtin_CEIL"].evalfn, "_starlight_decimal_lexical_form_patch", False):
        ...
        def _make_patched(name, transform):
            ...
            _patched._starlight_decimal_lexical_form_patch = True  # type: ignore[attr-defined]
            return _patched
...
def apply_all_operator_patches() -> None:
    """Apply both patches. Called eagerly from ``starlight/__init__.py`` so
