...
the originals do. All four go through ``sparql1_2_to_rdf``'s real
...
text-level rewriting:
  ...
  - ``parseQuery``/``parseUpdate`` return a raw parse tree (same shape
    ...
    via ``sparql1_2_to_rdf.parse12.parse_query_12``/``parse_update_12`` -
  ...
  - ``prepareQuery``/``prepareUpdate`` return a compiled, directly
    ...
    lowered to plain SPARQL 1.1 (``sparql1_2_to_rdf.lower_rdf11``'s
...
def parseQuery(q):
    ...
    """
    from sparql1_2_to_rdf.parse12 import parse_query_12
...
def prepareQuery(queryString: str, initNs=None, base=None):
    ...
    starlight's own ``tt:`` content-addressed encoding); the lowering is
...
def parseUpdate(q):
    ...
    """
    from sparql1_2_to_rdf.parse12 import parse_update_12
...
def processUpdate(graph, updateString: str, initBindings=None, initNs=None, base=None):
    ...
    For ``StarlightGraph``/``StarlightDataset`` instances, this delegates
    ...
    if hasattr(graph, '_tt_registry') or hasattr(graph, '_sg_cache'):
        ...
        # would flow through StarlightGraph.update()'s own **kwargs all the
        # way to SPARQLUpdateProcessor.update(), which doesn't accept it -
        # confirmed via a real TypeError. StarlightGraph.update() only
