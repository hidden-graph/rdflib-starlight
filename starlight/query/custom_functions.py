"""Registers starlight's own custom SPARQL extension functions with rdflib's
global function registry, as an import-time side effect.

These are the functions the SPARQL 1.2 -> 1.1 lowering (sparql1_2_to_rdf's
...
eagerly from ``starlight/__init__.py`` alongside this project's other
eager compatibility patches, so registration always happens regardless of
which pipeline a caller uses.

``starlight.query.remote_decompose`` imports the three function-IRI
...
decomposition - the literal same URIs ``sparql1_2_to_rdf.lower_rdf11``
produces, both deriving from ``starlight.model.encoding.TT_NS``/
``DIRLANG_NS``.
"""
...
def _register_tt_hash_function() -> None:
    ...
    def _tt_hash_fn(s, p, o):
        ...
        # them lets StarlightGraph._restore() reconstruct a proper
        # TripleTerm for a value that was computed but never written to
        # any graph. See starlight.model.encoding's _TT_HASH_MEMO docstring.
