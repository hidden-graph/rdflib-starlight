...
today — starlight never calls ``translateAlgebra``/``_AlgebraTranslator``
anywhere in its own pipeline (confirmed: no reference to either name in
``starlight/`` outside this file). All were found while building triple-term
...
query through starlight goes through, not just this specific downstream
...
def patch_algebra_translator_bugs() -> bool:
    ...
    try:
        ...
        if getattr(original_sparql_query_text, "_starlight_algebra_translator_patch", False):
        ...
        def _patched_sparql_query_text(self, node):
            ...
            if isinstance(node, CompValue) and node.name == "Extend":
                ...
                # stylistic: starlight's own `sparql12_to_11.py` rewriter
        ...
        _patched_sparql_query_text._starlight_algebra_translator_patch = True  # type: ignore[attr-defined]
