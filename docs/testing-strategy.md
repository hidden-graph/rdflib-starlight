...
SPARQL 1.2 query/update parsing goes through the sibling `sparql1_2_to_rdf`
package's real grammar/algebra pipeline (`starlight/query/query_cache.py`,
`starlight/query/sparql_api.py`) — a real, editable-installed dependency
...
other test modules was found and fixed during the migration off starlight's
