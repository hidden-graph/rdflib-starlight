...
| N-Triples/N-Quads `VERSION "1.2"` directive (confirmed via spec fetch: bare form, same grammar as SPARQL's) | N-Triples WD | **Fixed 2026-07-17.** Previously recognized only to discard it like a comment (`_parse_nt_line`) — the label was never extracted or checked. New `extract_version_directive()` (`ntriples12.py`) surfaces it; `StarlightGraph.parse()` runs the same conformance check as Turtle | ✅ Match |
| TriG document-level `VERSION` directive (applies to the whole document, not per `GRAPH` block) | inherits Turtle grammar | **Fixed 2026-07-17.** Previously silently dropped entirely — the per-`GRAPH`-block Turtle-parser calls never surfaced it. New `extract_version_directive()` (`trig12.py`) scans the whole document once; both `StarlightGraph.parse()` and `StarlightDataset.parse()` check it against the union of all resulting graphs | ✅ Match |
...
**`rdf:version` attribute — fixed 2026-07-17, and it turned out to be a real data-corruption bug, not just a missing feature.** Confirmed via spec fetch that RDF/XML's version mechanism is structurally different from every other format's (Turtle, N-Triples/N-Quads, TriG, SPARQL — see §1/§4): "features introduced in RDF 1.2 require a version announcement on an in-scope node element with an `rdf:version` attribute with a defined value other than `"1.1"`" — an XML attribute, not a prologue-line directive. Checked whether this was actually exercised by a real implementation before fixing it: **live Oxigraph 0.5.9 genuinely emits `rdf:version` in its RDF/XML output** (on the property element wrapping `rdf:parseType="Triple"` for a reified triple term, e.g. `<rdf:reifies rdf:version="1.2" rdf:parseType="Triple">`). Feeding that real output into starlight's own `rdfxml12.py` parser showed this wasn't merely unimplemented — it was silently **wrong**: rdflib's real `'xml'` parser treats any attribute it doesn't specifically recognize as ordinary RDF/XML "property attribute" shorthand, so an unstripped `rdf:version` asserted a bogus extra triple `(subject, rdf:version, "1.2")` into the graph. (Separately, Fuseki/Jena crashes with HTTP 500 trying to *write* a triple term to RDF/XML at all, so this specific interop concern is Oxigraph-only.) Fixed: `rdf:version` is now stripped wherever it's encountered during the existing XML-tree preprocessing (both node and property elements), and a new standalone `extract_version_directive()` surfaces the first value found for the same `RDF12ConformanceWarning` check every other format now has. Regression test embeds the literal captured Oxigraph bytes, not a hand-written approximation (`tests/unit/test_conformance.py::TestRdfXmlVersionAttribute::test_real_oxigraph_output_does_not_produce_bogus_triple`).
...
`starlight/serializers/jsonld12.py` is honest about this in its own docstring — it states the output is "valid JSON-LD 1.1" using an ad hoc `rdf:TripleTerm` node convention starlight defined itself, round-trippable only through starlight's own parser. Extended for `dirLangString` via an internal `dirlang:` datatype URI (`@type`) rather than JSON-LD 1.1's native `@direction` keyword — rdflib's JSON-LD codec (RDF 1.1) has no concept of `@direction` and would silently drop it on parse, so using `@type` instead keeps it round-trippable through rdflib's real, unmodified JSON-LD parser. Live-verified 2026-07-17 against both Fuseki 5.5.0 and Oxigraph: **neither serializes a triple term to JSON-LD at all** (Fuseki hard-crashes with an HTTP 500; Oxigraph returns an explicit "JSON-LD does not support RDF 1.2 yet"), so starlight's JSON-LD convention is presently ahead of, not behind, real-world implementations — genuinely speculative pending an actual JSON-LD RDF-1.2 revision (see `future_enhancements.md`'s "Keeping in step" section), but not contradicted by any existing tool either.

**`trix12.py` update 2026-07-17: this section's original verdict for TriX was wrong, not just imprecise.** It assumed "no external tool to round-trip through" — untested at the time. Live-checked against Fuseki 5.5.0: **Apache Jena has a real, working TriX writer/reader that does support RDF 1.2 triple terms** (Oxigraph has no TriX support at all, in either direction, confirmed separately). Starlight's original invented convention (`<TriX>` uppercase root, `<tripleTerm>` as a distinct element) was incompatible with Jena's actual output in both respects, confirmed by literally feeding Fuseki's live TriX output into starlight's own parser and watching it fail. **Fixed**: the serializer now emits Jena's exact convention (lowercase `<trix>` root; a triple term is a `<triple>` nested in a term position, reusing the same tag as an ordinary statement, disambiguated by structural position only) and the parser accepts it, while still accepting the old `<TriX>`/`<tripleTerm>` spelling for backward compatibility with anything already written by starlight's prior version. Re-verified live both directions: Fuseki accepts starlight's new TriX output unmodified, and starlight correctly parses Fuseki's real output byte-for-byte (`tests/unit/test_rdf12_formats.py::TestTriX12ParseJenaConvention::test_parses_real_fuseki_output_verbatim` uses the literal captured bytes, not a hand-written approximation).

**Verdict, updated**: `jsonld12` remains a genuine naming/expectation risk (no real implementation to compare against yet, in either direction) — the one-line README/docs callout already added stands. `trix12` is no longer in that category: it now interoperates with the one production implementation that matters for this format.
...
5. ~~**Version-directive validation, conformance levels** (§1, §2, §4)~~ — **done 2026-07-17, extended to every text-directive format the same day.** Re-checked against the live spec text and found a real bug hiding behind the "low priority" label: a SPARQL 1.2 query with a leading `VERSION "1.2"` directive — the spec's own example syntax — previously raised `ParseException` outright on the in-memory backend. Fixed (directive is now stripped before rewriting), and a new `RDF12ConformanceWarning` (warning-only, per the spec's explicit "merely a hint" language — never a hard error) fires when a document/query declares `"1.2-basic"`/`"1.1"` while still using a triple term or `dirLangString`, or declares an unrecognized label. Initially implemented for Turtle and SPARQL only; asking "does this extend to our other formats?" and actually checking (rather than assuming) turned up the same gap in N-Triples, N-Quads, and TriG (all fixed same day — see their own §2 rows) and a genuinely different, still-open one in RDF/XML's `rdf:version` attribute (see §3). Also found and fixed a related backend-consistency gap: `backend='rdf-1.2'` graphs never ran this check at all, since native-mode `query()`/`update()` bypass the rewriter it lived in — confirmed live against Fuseki/Oxigraph that neither engine surfaces any signal for a conformance mismatch either, so without the fix a native-backend graph would have silently never warned for a query the default backend does warn on. See `starlight/model/conformance.py` and `docs/future_enhancements.md`.
6. ~~**RDF/XML's `rdf:version` attribute** (§3)~~ — **done 2026-07-17.** Structurally different from every other format's directive-line mechanism (an XML attribute, confirmed via spec fetch), so not bundled into item 5's fix initially — then found, via live Oxigraph testing, that it's not just unimplemented but an active data-corruption bug: Oxigraph really emits `rdf:version`, and starlight's parser was turning it into a bogus extra triple rather than ignoring or checking it. Fixed the same day it was found.
7. **New: keeping in step with the spec as it moves from Candidate Recommendation to a final Recommendation** — this project's whole reason to exist is temporary by its own README's admission. See `docs/future_enhancements.md`'s "Keeping in step with RDF 1.2/SPARQL 1.2 as the spec finalizes" for the concrete re-check steps (re-run this gap analysis at each spec-stage transition, watch for a real JSON-LD RDF-1.2 companion spec, re-evaluate this project's continued existence once rdflib ships native support).
