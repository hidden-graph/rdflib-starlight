...
| Version/conformance levels ("1.2", "1.2-basic", "1.1") | §2, §2.1 | Not modeled — starlight has no concept of a "basic" (no-triple-terms) conformance mode; it's all-or-nothing | ⚠️ Minor gap, low priority (no known use case blocked) |
...
| `@version "1.2" .` / `VERSION "1.2"` directive | Turtle §[6],[9] | Parsed and silently discarded (`turtle_parser.py:435`, `syntax.py:139-140`) — no validation that declared version matches features actually used, no distinction between the dotted and dot-less forms | ⚠️ Minor gap — cosmetic; doesn't affect data correctness since starlight accepts RDF 1.2 syntax unconditionally regardless of the directive |
...
| `VERSION "1.2"` / `-1.2-basic` query prologue directive | Not handled by `sparql12_to_11.py` | ⚠️ Minor — same posture as the Turtle version directive: starlight rewrites RDF 1.2 syntax unconditionally, so an absent/ignored directive doesn't break anything, it just isn't validated |
...
5. **Version-directive validation, conformance levels** (§1, §2) — low priority, no known blocked use case.
