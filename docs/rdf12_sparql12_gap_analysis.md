...
**New, not yet fixed (found 2026-07-17): the `rdf:version` attribute.** Unlike every other format's VERSION support (Turtle, N-Triples/N-Quads, TriG, SPARQL — all fixed the same day, see §1/§4), RDF/XML doesn't use a prologue-line directive at all — confirmed via spec fetch: "features introduced in RDF 1.2 require a version announcement on an in-scope node element with an `rdf:version` attribute with a defined value other than `"1.1"`." This is a structurally different mechanism (an XML attribute on a node element, not a text directive at the top of the document), so it doesn't fit the `extract_version_directive()` pattern reused everywhere else — it needs its own detection logic scanning node-element attributes in `rdfxml12.py`. Not implemented at all currently. Deliberately deferred as a separate follow-up rather than bundled into the other fixes.
...
6. **New, open: RDF/XML's `rdf:version` attribute** (§3) — structurally different from every other format's directive-line mechanism (an XML attribute on an in-scope node element, confirmed via spec fetch), so it wasn't bundled into item 5's fix. Not implemented at all currently.
