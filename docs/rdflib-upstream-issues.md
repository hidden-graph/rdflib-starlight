...
rdflib 7.6.0. Found while building SPARQL 1.2 support in this repo; every repro is plain `rdflib`, no starlight code. **Not filed upstream** - deferred until rdflib itself starts RDF 1.2 work, then reported as a batch rather than piecemeal now. All have a real fix applied in this repo.
...
Fix: `starlight/query/operator_patches.py::patch_multiplicative_expression_type_promotion`.
...
Fix: `starlight/query/algebra_translator_patches.py::patch_algebra_translator_bugs`.
...
Fix: `starlight/query/evaluate_patches.py::patch_evalextend_forgotten_bind_vars`.
...
Fix: `starlight/query/evaluate_patches.py::patch_lazy_join_expr_dependency_order`.
...
Fix: `starlight/query/evaluate_patches.py::patch_group_by_unaliased_expression_key`.
...
Fix: `starlight/query/result_patches.py::patch_result_iter_empty_binding_row`.
